...
type Distortions = Record<string, Distortion>;
...
class LightsSticks {
  ...
  init() {
    ...
    if (Array.isArray(options.colors.sticks)) {
      colorArray = (options.colors.sticks as unknown as number[]).map((c) => new THREE.Color(c));
    } else {
...
export function Hyperspeed({ effectOptions = DEFAULT_EFFECT_OPTIONS }: HyperspeedProps) {
  ...
  return <div className="hyperspeed-container" ref={hyperspeed} />;
}
